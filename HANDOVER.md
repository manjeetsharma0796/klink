...
- **All 8 instructions live on Solana devnet.** Program ID `DPPE8TAuw5qyWbw5MqcXcAtH2d5RYF5XBXTiN2pKzM3L`, redeployed `2026-05-11` from build commit `b4aa1bb` (T-252 TransferChecked migration + new program id). Full provenance in [`docs/runbooks/devnet-deploys.md`](docs/runbooks/devnet-deploys.md).
- **Old program ID `5qCJCEhfLusk59YFqaEG9Yg3Wp64ZaYwvXteFmCmedqv` is abandoned and unreachable.** Its single-signer upgrade authority (`6fEL…drjZ`, held by @Prithwish) was lost when his laptop was lost — no backup. ~1.93 SOL of ProgramData rent is stuck there forever. Every devnet vault/session/Kamino position derived from the old id is functionally orphaned (still on-chain, but only reachable through the frozen pre-TransferChecked program). For devnet that was acceptable; for mainnet it would have been catastrophic.
- **Deploy authority is still a single keypair** (`3kk1MijUXnxt9YDrNTWe2QLF8rsidvNsr7Nkba8Qb4ik` as of `2026-05-11`). **T-114 (Squads 2-of-N rotation) is now blocker-level urgent**, not backlog — the keypair-loss path that took out the original deploy authority is exactly what multisig is supposed to prevent. **Do not promote to mainnet** until T-114 lands.
