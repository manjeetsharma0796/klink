...
- Reverted: 2026-05-11 by @Jishnu — Manish's claim 12 days stale; zero test files written (only a planning doc `programs/agent_wallet/tests/T-116-cases.md`); taken over per `docs/runbooks/team-collaboration.md §2.2`.
- Depends-on: T-105, T-106, T-107
- OS: any
- Scope: tests
- Acceptance: 7 of 8 named tests from spec §6.1.1 land green; 8th (`kamino_deposit_reverts_when_over_max_deployed_fraction`) is `#[ignore]` with a documented Kamino-program-mock blocker. Tests live as Rust integration tests under `tests/tests/` and run via `cargo test --workspace` in the existing `anchor-ci.yml` workflow. Wall-clock: each test 0.34–0.41s, total suite under 4s. Toolchain: `litesvm = "0.11"` + `solana-sdk = "3"` + `borsh = "1.5"` (replaces deprecated `try_to_vec` with `borsh::to_vec`); SPL Token + ATA program ids referenced by string literal to avoid pulling spl-* into the dep graph (their transitive `curve25519-dalek` pin conflicts with `solana-zk-sdk`'s zeroize requirement). Reusable fixture at `tests/tests/common/mod.rs` provides `Fixture::new()` (boots litesvm, loads `target/deploy/agent_wallet.so`, funds payer + owner), `init_vault`, `add_session`, `create_usdc_accounts` (writes raw SPL Mint + TokenAccount bytes via `LiteSVM::set_account`, no SPL Token program invocation), `transfer_usdc`, `revoke_session`, `warp_clock_to`, and the `assert_anchor_error` helper that maps the user error variants to their Anchor 6000+ offsets. Test files: `revert_amount_exceeds_max_per_tx.rs`, `revert_recipient_not_in_allowlist.rs`, `revert_daily_cap_exceeded.rs`, `revert_session_revoked.rs`, `revert_session_expired.rs`, `revert_only_owner_can_revoke.rs`, `race_only_first_wins_on_cap.rs`, `revert_kamino_over_max_deployed.rs` (the ignored one with its TODO inline). Unblocks T-111 + T-112. Plan + traceability at `docs/superpowers/plans/2026-05-11-t110-revert-suite.md`.
- Notes: shipped via parallel-subagent execution. Phase 1 (scaffold + first test) sequential by @Jishnu; Phase 2 dispatched 3 Explore agents in parallel — agent A wrote tests 2+3, agent B wrote tests 4+5+7, agent C wrote tests 6+8. Total wall-clock: ~90 min including a Solana 3.x version-skew detour (litesvm 0.7 pulled solana-pubkey 2.4 internally, incompatible with the 3.x family; bumped to litesvm 0.11 which speaks solana 3 cleanly). Out of scope: kamino_deposit revert needs a mock of Kamino's lending program to satisfy Anchor's account validation before the in-handler math check fires; filed implicitly as a follow-up since it's a single test + ~50 LOC of Kamino fake-account scaffolding. T-115 external audit is the next gate before mainnet; T-114 multisig is deferred per user direction.
