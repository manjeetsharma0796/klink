...
- OS: any
- Scope: api + ops
- Acceptance: live `POST /v1/yield/deposit` and `POST /v1/yield/withdraw` against `klink-api.onrender.com` currently return `500 "server misconfigured"`. Root cause: the 5 Kamino env vars `apps/api/src/routes/yield.ts:55-59` reads via `envPubkey()` (`KAMINO_RESERVE`, `KAMINO_LENDING_MARKET`, `KAMINO_LENDING_MARKET_AUTHORITY`, `KAMINO_RESERVE_LIQUIDITY_SUPPLY`, `KAMINO_RESERVE_COLLATERAL_MINT`) are unset on the Render service env. T-403's notes claim "Kamino" was configured on Render via the Render API, but the live 500 contradicts that — likely cleared, or only the program-id constant got set and not the per-reserve PDAs. Steps: (a) confirm the 5 expected vars from `apps/api/.env.example:40-44` (empty placeholders today); (b) source the canonical devnet pubkey values for the Kamino USDC reserve currently in use locally — same values that pass local e2e; (c) set all 5 in the Render dashboard env editor for the `klink-api` service; (d) trigger a redeploy or wait for the next push to `main`; (e) smoke-test by `curl -X POST -H "Authorization: Bearer $KLINK_API_KEY" -d '{"amount":1000000}' https://klink-api.onrender.com/v1/yield/deposit` against the funded test wallet (currently 17.5 USDC liquid per `/v1/yield/position`) — expect `200 {"tx_signature":"...","status":"confirmed"}`. Side cleanup: file the 5 env vars in `render.yaml` (or a scripted equivalent) so the config is reproducible — T-403 already flagged "no `render.yaml` checked in" as a follow-up; this is the natural moment to address that for the Kamino vars at minimum. Skill.md "Beta caveats" already references this task ID and tells agents not to retry until it lands.
- Notes: surfaced 2026-05-04 by a fresh-context agent validation pass against the deployed api. Auth, validation, transfer, position-read, error taxonomy all confirmed working — yield deposit/withdraw are the only broken endpoints, and the failure mode (`500 "server misconfigured"`) is exactly what `apps/api/src/routes/yield.ts` returns when `envPubkey()` throws. Whoever takes this needs Render dashboard access + the canonical Kamino devnet reserve pubkeys.
