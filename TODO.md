...
- Acceptance: surfaced 2026-05-11 by smoking T-252 against a real MPP devnet echo at `https://service01-kep9.onrender.com/echo`. The on-chain side passed (TransferChecked opcode 12 emitted, vault PDA derived from new program `DPPE8…zM3L`, tx finalized, 0.01 USDC moved on-chain) but the MPP verifier still returned `402 verification-failed`. Two integration gaps in klink, both proven by an end-to-end loop with tx decode: (1) `/v1/spend/sign-payment` calls `getLatestBlockhash` server-side and ignores any caller-supplied recentBlockhash — MPP requires the on-chain tx to bind to the challenge's specific recentBlockhash so the merchant can prove this payment is fresh for *this* challenge (anti-replay); the klink tx's blockhash and the challenge's blockhash were different in every probe. (2) skill.md tells the agent to send `X-Payment-Proof: <sig>` x402-style; MPP explicitly says *"Do NOT use headers like X-Payment-Proof or X-Payment - only `Authorization: Payment` is read"*. The token shape MPP demands is `base64url(JSON.stringify({challenge, payload:{type:"signature", signature}}))`. New `postSpendMppHandler` in `apps/api/src/routes/spend.ts` mirrors `postSpendServiceHandler`'s probe→sign→retry→forward proxy pattern but speaks MPP: probe URL, expect `402 + WWW-Authenticate: Payment id="…", request="<base64url>"`, parse the challenge into a structured object, decode `request` to extract `amount` / `recipient` / `methodDetails.recentBlockhash` / `currency`, gate on session policy (URL allowlist via T-209's `checkOffChainPolicy`, recipient allowlist + max_per_tx + daily cap on-chain, plus `Date.now() < challenge.expires` for freshness), decrypt session keypair, build TransferChecked tx using the *challenge's* recentBlockhash (not `getLatestBlockhash`), sign + submit, build `Authorization: Payment <base64url(JSON.stringify({challenge, payload:{type:"signature", signature}}))>` header, retry the same URL with that header, forward upstream response (status, content-type, body) + `x-tx-signature` header back to the agent. New deny taxonomy rows in audit_log: `NO_402` (upstream didn't return Payment-Required), `BAD_CHALLENGE` (WWW-Authenticate unparseable or missing required fields), `EXPIRED_CHALLENGE` (`Date.now() ≥ challenge.expires`), `VERIFICATION_FAILED` (upstream rejected retry; money already moved → 502 to agent with the upstream body, mirrors T-211's `RETRY_FAILED` semantic). Tests pin all the above + a blockhash-propagation assertion (built tx's `recentBlockhash` MUST equal `challenge.request.methodDetails.recentBlockhash`). New section in both `apps/web/public/skill.md` and `gitbook/skill.md` (byte-equal sync test enforces) explaining when to use `/v1/spend/mpp` vs `/v1/spend/sign-payment` (the latter for x402-style services where the agent retries; the former for MPP services where klink retries server-side). End-to-end smoke against `https://service01-kep9.onrender.com/echo` must return `200 OK + Payment-Receipt` for close.
