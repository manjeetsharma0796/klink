...
- Acceptance: full plan at `docs/superpowers/plans/2026-05-11-transfer-checked-migration.md` (8 tasks, ~4h). Migrate `programs/agent_wallet/src/instructions/transfer_usdc.rs:1-158` from plain SPL `Transfer` (opcode 3) to `TransferChecked` (opcode 12). Adds `mint: Account<'info, Mint>` to `TransferUsdc<'info>` (between `vault` and `vault_usdc_ata`); CPI becomes `token::transfer_checked(ctx, amount, mint.decimals)` with decimals read from the supplied Mint account on-chain (canonical SPL pattern, no new caller arg). `apps/api/src/program/agent-wallet.ts:273 buildTransferUsdcIx` extends from 6 to 7 keys with mint at index 3. The 3 spend handlers in `apps/api/src/routes/spend.ts` (`postSpendTransferHandler` near L240, `postSpendSignPaymentHandler` near L442, `postSpendServiceHandler` near L732) pass `mint: getUsdcMint()`. Existing tests at `apps/api/tests/program/agent-wallet.test.ts` updated for 7-key shape; api test count unchanged. Devnet redeploy under existing single-signer authority for program `5qCJCEhfLusk59YFqaEG9Yg3Wp64ZaYwvXteFmCmedqv`; runbook row appended to `docs/runbooks/devnet-deploys.md`. End-to-end smoke against MPP devnet echo with the wallet that failed 5x at the verifier on 2026-05-11 must return 200 OK + `Payment-Receipt` header for close. Mainnet gated on T-114 multisig rotation. `owner_transfer_usdc` keeps plain `Transfer` for now (escape hatch is not verifier-bound); separate follow-up filed implicitly.
- Notes: triggered 2026-05-11 by direct probe against MPP devnet echo with klink dev key. 5 spend-transfer attempts at 10000 base units each landed on-chain successfully but were every-time rejected by the MPP verifier at `@solana/mpp/src/server/Charge.ts:340-343` (filter `parsed?.type === 'transferChecked'`) with the error string "No TransferChecked instruction found for recipient ..." surfacing as generic 402. HMAC, base58 signature, amount, recipient, mint all correct in the failed probes; only the SPL ix opcode was wrong (3 vs 12). Live balance at probe end via `GET /v1/yield/position`: 9.964506 USDC (0.05 USDC burned across the 5 retries). MPP requires `TransferChecked` because it carries mint + decimals on-wire, removing the need for racy ATA lookups during off-chain verification. The same requirement applies to x402 verifiers (PayAI Solana facilitator), so this migration unlocks both protocols simultaneously without further parser work. Reference working client that already uses `@solana/mpp` SDK with `TransferChecked`: `D:/workspace/mpp-mock-service/client/pay-echo.ts`.
